...
def get_admin_keyboard() -> ReplyKeyboardMarkup:
    ...
    builder.button(text="Редактировать товары")
    builder.button(text="Заявки на оплату")
...
def get_categories_keyboard(categories: List[Dict]) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        emoji_part = f'<tg-emoji emoji-id="{cat["emoji_id"]}"></tg-emoji> ' if cat.get("emoji_id") else ""
        buttons.append([InlineKeyboardButton(
            text=f"{emoji_part}{cat['name']}",
...
def get_products_keyboard(products: List[Dict], category_id: int) -> InlineKeyboardMarkup:
    ...
    for product in products:
        emoji_part = f'<tg-emoji emoji-id="{product["emoji_id"]}"></tg-emoji> ' if product.get("emoji_id") else ""
        stock = "" if product['quantity'] == -1 else f" ({product['quantity']} шт.)"
        buttons.append([InlineKeyboardButton(
            text=f"{emoji_part}{product['name']} - {product['price']}₽{stock}",
...
# ==================== ФУНКЦИИ ПОМОЩНИКИ ====================
def format_message_with_emoji(text: str, emoji_id: str) -> str:
...
async def create_crypto_invoice(amount_rub: float, description: str) -> Optional[Dict]:
    # Конвертация RUB в USDT по курсу 1 USDT = 90 RUB
...
async def cmd_admin(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("<tg-emoji emoji-id=\"" + EMOJI['cross'] + "\"></tg-emoji> У вас нет доступа к админ-панели.")
        return
...
async def admin_edit_products(message: Message):
    ...
    if not products:
        await message.answer("<b>Нет товаров для редактирования.</b>")
        return
...
async def admin_pending_orders(message: Message):
    ...
    if not orders:
        await message.answer("<b>Нет заявок на рассмотрение.</b>")
        return
    
    for order in orders:
        order_text = f"""<b>Заявка #{order['id']}</b>
...
async def show_category_products(callback: CallbackQuery):
    ...
    if not products:
        await callback.message.edit_text(
            "<b>В этой категории пока нет товаров.</b>",
...
async def show_product_detail(callback: CallbackQuery):
    ...
    detail_text = f"""<b>{emoji_part}{product['name']}</b>
...
async def pay_crypto(callback: CallbackQuery):
    ...
    await db.update_order(order_id, 'pending_crypto')
    await db.pool.acquire().then(lambda conn: conn.execute(
        "UPDATE orders SET crypto_invoice_id = $1 WHERE id = $2",
        invoice['invoice_id'], order_id
    ).then(lambda _: conn.close()))
    
    pay_text = f"""<b>Оплата через Crypto Bot</b>
...
async def pay_sbp(callback: CallbackQuery, state: FSMContext):
    ...
    await db.update_order(order_id, 'pending_sbp')
    
    sbp_text = f"""<b>Оплата через СБП</b>

<tg-emoji emoji-id="{EMOJI['money_send']}"></tg-emoji> Сумма: {order['total_price']} ₽
<tg-emoji emoji-id="{EMOJI['phone'] if 'phone' in EMOJI else EMOJI['wallet']}"></tg-emoji> Номер телефона: +79818376180
<tg-emoji emoji-id="{EMOJI['bank'] if 'bank' in EMOJI else EMOJI['wallet']}"></tg-emoji> Банк: ЮMONEY
...
async def pay_card(callback: CallbackQuery, state: FSMContext):
    ...
    await db.update_order(order_id, 'pending_card')
    
    card_text = f"""<b>Оплата картой</b>
...
async def mark_as_paid(callback: CallbackQuery, state: FSMContext):
    ...
    if order['payment_method'] == 'crypto':
        # Проверяем статус крипто-платежа
        ...
        # Для СБП и карты - ждем чек
        await callback.message.edit_text(
            "<b>Отправьте скриншот/чек об оплате.</b>\n\nАдминистратор проверит платеж и подтвердит заказ.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def complete_order(callback: CallbackQuery, order_id: int, order: Dict):
    ...
    await db.update_order(order_id, 'completed')
...
async def receive_payment_proof(message: Message, state: FSMContext):
    ...
    for admin_id in ADMIN_IDS:
        try:
            admin_text = f"""<b>Новая заявка на оплату #{order_id}</b>
...
async def reject_order(callback: CallbackQuery):
    ...
    await db.update_order(order_id, 'rejected')
...
async def edit_product_callback(callback: CallbackQuery):
    ...
    detail_text = f"""<b>Редактирование товара</b>

{emoji_part}<b>{product['name']}</b>
{product['description']}
...
@router.message(StateFilter(EditProductStates.waiting_for_value))
...
async def add_product_category(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "<b>Введите название товара (можно с премиум эмодзи):</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def add_product_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>Введите описание товара (можно с премиум эмодзи):</b>"
    )
...
async def add_product_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>Введите количество товара (-1 для неограниченного):</b>"
    )
...
async def add_product_quantity(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>Выберите тип контента товара:</b>",
...
async def add_product_content_type(callback: CallbackQuery, state: FSMContext):
    ...
    if content_type == 'text':
        ...
        await callback.message.edit_text(
            "<b>Введите текст, который получит покупатель:</b>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="Отмена", callback_data="cancel_add_product")]
            ])
        )
        ...
        await callback.message.edit_text(
            f"<b>Отправьте {content_type} (до 10 штук). Когда закончите, нажмите /done</b>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def add_product_text_content(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>Отправьте ID премиум эмодзи для товара (или 0 если не нужно):</b>"
    )
...
async def add_product_media(message: Message, state: FSMContext):
    ...
    if len(media_files) >= 10:
        await message.answer("<b>Максимум 10 файлов. Нажмите /done</b>")
        return
    
    media_files.append(file_id)
    await state.update_data(media_files=media_files, caption=caption)
    
    await message.answer(f"<b>Файл добавлен ({len(media_files)}/10). Отправьте еще или нажмите /done</b>")

@router.message(Command("done"), StateFilter(AddProductStates.waiting_for_media))
async def add_product_media_done(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    ...
    if not media_files:
        await message.answer("<b>Добавьте хотя бы один файл!</b>")
        return
...
async def cancel_add_product(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(
        "<b>Добавление товара отменено.</b>"
    )
    await callback.message.answer(
        "<b>Админ-панель</b>",
        reply_markup=get_admin_keyboard()
